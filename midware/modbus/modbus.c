...
*/
          modH->xTimerTimeout=osTimerNew(vTimerCallbackTimeout, osTimerOnce, NULL); 
		  if(modH->xTimerTimeout == NULL)
		  {
			  while(1); //error creating timer, check heap and stack size
		  }
          osTimerStart(modH->xTimerTimeout, modH->u16timeOut);
...
*/
      modH->xTimerT35=osTimerNew(vTimerCallbackT35, osTimerOnce, NULL); 
                                    
	  if (modH->xTimerT35 == NULL)
	  {
		  while(1); //Error creating the timer, check heap and stack size
	  }
      osTimerStart(modH->xTimerTimeout, T35);
  ...
  for(;;)
  {
   ...
   // check slave id
	...
	if (u8exception > 0)
	{
	 ...
	 modH->i8lastError = 0;
	osSemaphoreAcquire(modH->ModBusSphrHandle , osWaitForever); //before processing the message get the semaphore
	 ...
	 osSemaphoreRelease(modH->ModBusSphrHandle); //Release the semaphore
...
void StartTaskModbusMaster(void *argument)
{
  ...
  for(;;)
  {
     ...
     //ulNotificationValue = ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
	  ...
	  osTimerStop(modH->xTimerTimeout); // cancel timeout timer
	  ...
	  {
		  osSemaphoreRelease(modH->ModBusSphrHandle); //Release the semaphore
		  //xTaskNotify((osThreadId_t)telegram.u32CurrentTask, ERR_OK_QUERY, eSetValueWithOverwrite);
	  }
