...
/* TODO: 替换为自己设备的三元组 */
char *product_key       = "izenKGgdF0A";
char *device_name       = "lab_1";
char *device_secret     = "a53fff307f9da16b629868f5232adec7";
...
*/
char  *mqtt_host = "iot-06z00cq4vq4hkzx.mqtt.iothub.aliyuncs.com";
/* 
...
void user_ota_recv_handler(void *ota_handle, aiot_ota_recv_t *ota_msg, void *userdata)
{
    ...
    case AIOT_OTARECV_FOTA: {
        uint16_t port = 443;
        uint32_t max_buffer_len = ONCE_PKT_LEN;
...
static int download_poll(void *handle, uint32_t len)
{
    int r;
    uint32_t end=g_dl_length+len;
    
    aiot_download_setopt(handle, AIOT_DLOPT_RANGE_START, (void *)&g_dl_length);
    aiot_download_setopt(handle, AIOT_DLOPT_RANGE_END, (void *)&end);
    r = aiot_download_send_request(handle);
    if(r==STATE_SUCCESS) {
        r = aiot_download_recv(handle);
        if(r>0) {
            g_dl_length += r;
            
            printf("___ cur_len: %d, g_dl_length: %d\n", r, g_dl_length);
        }
        else {
            printf("___ aiot_download_recv -0x%04x\n", -r);
        }
    }
    else {
        printf("___ aiot_download_send_request failed -0x%04x\n", -r);
    }
...
void fota_demo_fn(void *arg)
{
    ...
    while (1) {
        ...
        if (NULL != g_dl_handle) {
            ...
            int ret = download_poll(g_dl_handle, ONCE_PKT_LEN);
            ...
            if (ret <= STATE_SUCCESS) {
                printf("download failed, error code is -0x%x, try to send renewal request\r\n", -ret);
                continue;
            }
        }
    }
