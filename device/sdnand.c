...
/* Includes ------------------------------------------------------------------*/
#include "dev/sdnand.h"

#define logf(x)   //Message_Error_No_Title(x);//VCP_DataTx_Ascii(x)
...
SD_Error SD_Init(void)
{
  ...
  errorstatus = SD_PowerON();
	...
	{
		char dispbuf[256];
		sprintf(dispbuf,"!!!INIT SD CARD 1!!!",(int)0);
		logf (dispbuf);
	}
		logf ("!!!INIT SD CARD 1!!!");
...
DSTATUS TM_FATFS_SD_SDIO_disk_initialize(void) {
	...
	{
		char dispbuf[256];
		sprintf(dispbuf,"!!!INIT SD CARD %d!!!",(int)TM_FATFS_SD_SDIO_Stat);
		logf (dispbuf);
	}
...
DRESULT TM_FATFS_SD_SDIO_disk_read(BYTE *buff, DWORD sector, UINT count) {
	...
	__align(4) BYTE wrbuf[2048];//一定要4字节对齐 因为用到DMA了
	...
	if ((TM_FATFS_SD_SDIO_Stat & STA_NOINIT)) {
		...
		logf ("SD read 1");
	SD_ReadMultiBlocks(wrbuf, sector << 9, 512, count);
	...
	{}
		...
		logf ("SD read 4");
	if (status == SD_OK) {
			memcpy(buff,wrbuf,count*512);
		logf ("SD read ok\r\n");
		return RES_OK;
	}
	{
		char dispbuf[256];
		sprintf(dispbuf,"!!!read SD CARD ERROR %d!!!",(int)status);
		logf (dispbuf);
	}
	return RES_ERROR;
}
...
DRESULT TM_FATFS_SD_SDIO_disk_write(BYTE *buff, DWORD sector, UINT count) {
	...
	__align(4) BYTE wrbuf[2048];//一定要4字节对齐 因为用到DMA了
		...
		memcpy(wrbuf,buff,count*512);

	SD_WriteMultiBlocks((BYTE *)wrbuf, sector << 9, 512, count);
