...
const unsigned char HAL_MSDC_EINT = HAL_EINT_NUMBER_14; //ian changed
#endif
...
{
//    printf("call msdc_sdio_interrupt_handle\r\n");
...
void msdc_sdio_interrupt_set(msdc_port_t msdc_port,bool enable)
{
    ...
    msdc_register_base = (MSDC_PORT_0 == msdc_port) ? MSDC0_REG : MSDC1_REG;
    if (enable) {
        msdc_register_base->MSDC_CFG |=  MSDC_CFG_INTEN_MASK;
//        msdc_register_base->MSDC_CFG |=  MSDC_CFG_DIRQEN_MASK;
...
//        sdio_isr();
        msdc_sdio_interrupt_handle(MSDC_PORT_0);//ian added
...
//    printf("irq handler got msdc_int=%08X after read msdc_int=%08X  MSDC_STA =%08X  SDIO_STA =%08X   SDIO_CFG =%08X\r\n", 
//             msdc_config[MSDC_PORT_1].interrupt_register,MSDC1_REG->MSDC_INT,MSDC1_REG->MSDC_STA, MSDC1_REG->SDIO_STA,MSDC1_REG->SDIO_CFG);
...
//    if (msdc_config[MSDC_PORT_1].interrupt_register & MSDC_INT_SDIOIRQ_MASK) {
    if ((msdc_config[MSDC_PORT_1].interrupt_register & MSDC_INT_SDIOIRQ_MASK)&& MSDC1_REG->SDIO_STA) {
//        sdio_isr();
//        MSDC1_REG->SDIO_CFG &= ~SDIO_CFG_INTEN_MASK;//IAN ADD FOR DISABLE SDIO INT
        msdc_sdio_interrupt_handle(MSDC_PORT_1);//ian added
    }
#endif
...
void msdc_eint_isr(void *user_data)
{
    //printf("msdc0_eint_isr triggered \r\n");
    hal_eint_mask((hal_eint_number_t)HAL_MSDC_EINT);
...
void msdc1_eint_isr(void *user_data)
{
//    printf("msdc1_eint_isr triggered \r\n");
    hal_eint_mask((hal_eint_number_t)HAL_MSDC_EINT);

    msdc_sdio_interrupt_handle(MSDC_PORT_1);
