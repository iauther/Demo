...
static sp_return_t get_config(sp_port_t *port, sp_data_t *data,
	sp_config_t *config);

static sp_return_t set_config(sp_port_t *port, sp_data_t *data,
	const sp_config_t *config);
...
SP_API sp_return_t sp_get_port_by_name(const char *portname, sp_port_t **port_ptr)
{
	...
	*port_ptr = NULL;
	...
	if (!(port = malloc(sizeof(sp_port_t))))
...
SP_API sp_return_t sp_get_port_usb_vid_pid(const sp_port_t *port,
...
SP_API sp_return_t sp_get_port_handle(const sp_port_t *port,
                                         void *result_ptr)
{
...
SP_API sp_return_t sp_copy_port(const sp_port_t *port,
                                   sp_port_t **copy_ptr)
{
...
SP_PRIV sp_port_t **list_append(sp_port_t **list,
                                     const char *portname)
{
	void *tmp;
	size_t count;
	...
	for (count = 0; list[count]; count++)
		;
	if (!(tmp = realloc(list, sizeof(sp_port_t *) * (count + 2))))
		goto fail;
	list = tmp;
...
SP_API sp_return_t sp_list_ports(sp_port_t ***list_ptr)
{
	...
	*list_ptr = NULL;
	...
	if (!(list = malloc(sizeof(sp_port_t *))))
...
SP_API void sp_free_port_list(sp_port_t **list)
{
	unsigned int i;
...
static sp_return_t restart_wait(sp_port_t *port)
{
	DWORD wait_result;
...
SP_API sp_return_t sp_open(sp_port_t *port, sp_mode_t flags)
{
	sp_data_t data;
	sp_config_t config;
	sp_return_t ret;
...
#ifdef _WIN32
static sp_return_t await_write_completion(sp_port_t *port)
{
	TRACE("%p", port);
	DWORD bytes_written;
	BOOL result;
...
SP_API sp_return_t sp_blocking_write(sp_port_t *port, const void *buf,
...
SP_API sp_return_t sp_nonblocking_write(sp_port_t *port,
...
static sp_return_t restart_wait_if_needed(sp_port_t *port, unsigned int bytes_read)
{
	DWORD errors;
	COMSTAT comstat;
...
SP_API sp_return_t sp_blocking_read(sp_port_t *port, void *buf,
                                       size_t count, unsigned int timeout_ms)
{
	TRACE("%p, %p, %d, %d", port, buf, count, timeout_ms);
...
SP_API sp_return_t sp_blocking_read_next(sp_port_t *port, void *buf,
                                            size_t count, unsigned int timeout_ms)
{
	TRACE("%p, %p, %d, %d", port, buf, count, timeout_ms);
...
SP_API sp_return_t sp_nonblocking_read(sp_port_t *port, void *buf,
                                          size_t count)
{
	TRACE("%p, %p, %d", port, buf, count);
...
SP_API sp_return_t sp_new_event_set(struct sp_event_set **result_ptr)
{
...
static sp_return_t add_handle(struct sp_event_set *event_set,
		event_handle handle, sp_event_t mask)
{
	void *new_handles;
	sp_event_t *new_masks;
	...
	if (!(new_masks = realloc(event_set->masks,
			sizeof(sp_event_t) * (event_set->count + 1))))
...
SP_API sp_return_t sp_add_port_events(struct sp_event_set *event_set,
...
SP_API sp_return_t sp_wait(sp_event_set_t *event_set,
...
static sp_return_t get_baudrate(int fd, int *baudrate)
{
	void *data;
...
static sp_return_t set_baudrate(int fd, int baudrate)
{
	void *data;
...
#ifdef USE_TERMIOX
static sp_return_t get_flow(int fd, sp_data_t *data)
{
	void *termx;
...
static sp_return_t set_flow(int fd, sp_data_t *data)
{
	void *termx;
...
SP_API sp_return_t sp_new_config(sp_config_t **config_ptr)
{
	...
	*config_ptr = NULL;
	...
	if (!(config = malloc(sizeof(sp_config_t))))
...
SP_API sp_return_t sp_get_config(sp_port_t *port,
                                    sp_config_t *config)
{
	sp_data_t data;
...
SP_API sp_return_t sp_set_config(sp_port_t *port,
                                    const sp_config_t *config)
{
	sp_data_t data;
	sp_config_t prev_config;
...
SP_API sp_return_t sp_set_##x(sp_port_t *port, type x) { \
	sp_data_t data; \
	sp_config_t config; \
	TRACE("%p, %d", port, x); \
	CHECK_OPEN_PORT(); \
...
SP_API sp_return_t sp_get_config_##x(const sp_config_t *config, \
                                        type *x) { \
	TRACE("%p, %p", config, x); \
	if (!x) \
		RETURN_ERROR(SP_ERR_ARG, "Null result pointer"); \
...
SP_API sp_return_t sp_set_config_##x(sp_config_t *config, \
                                        type x) { \
	TRACE("%p, %d", config, x); \
	if (!config) \
		RETURN_ERROR(SP_ERR_ARG, "Null config"); \
...
CREATE_ACCESSORS(parity, sp_parity_t)
CREATE_ACCESSORS(stopbits, int)
CREATE_ACCESSORS(rts, sp_rts_t)
CREATE_ACCESSORS(cts, sp_cts_t)
CREATE_ACCESSORS(dtr, sp_dtr_t)
CREATE_ACCESSORS(dsr, sp_dsr_t)
CREATE_ACCESSORS(xon_xoff, sp_xonxoff_t)

SP_API sp_return_t sp_set_config_flowcontrol(sp_config_t *config, 
...
SP_API sp_return_t sp_set_flowcontrol(sp_port_t *port,
                                         sp_flowcontrol_t flowcontrol)
{
	sp_data_t data;
	sp_config_t config;
...
SP_API sp_return_t sp_get_signals(sp_port_t *port,
                                     sp_signal_t *signals)
{
