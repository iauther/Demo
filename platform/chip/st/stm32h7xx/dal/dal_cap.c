...
int dal_cap_config(dal_cap_cfg_t *cfg)
{
    ...
    sc.adc.samples = ADC_SAMPLE_POINTS;
    sc.adc.callback = capCfg.adc_fn;
    dal_adc_config(&sc.adc);

    sc.sai.e3Mux = capCfg.e3Mux;
    sc.sai.chBits = capCfg.chBits;
    
    sc.sai.mdma.chs = capCfg.chs;
    sc.sai.mdma.chX = capCfg.chs;
    sc.sai.mdma.chY = 0;
    ...
    dal_sai_config(&sc.sai);
    
    //sc.tmr = 0;
    //dal_tmr_config(&sc.tmr);
