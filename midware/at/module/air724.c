...
static int air724_cscon_close(void)
{
    ...
    at_echo_create(&echo, NULL, 0, NULL);
     while  (try++ < 10) {
        at_cmd_exec(&echo, 300, "AT+CSCON=0\r\n");
        if (echo.status == AT_ECHO_STATUS_OK) {
            return 0;
        }
        sleep_ms(1000);
...
static int air724_gprs_network_check(void)
{
    ...
    while (try++ < 10) {
        at_echo_create(&echo, echo_buffer, sizeof(echo_buffer), NULL);
        at_cmd_exec(&echo, 1000, "AT+CGREG?\r\n");
        if (echo.status != AT_ECHO_STATUS_OK) {
            return -1;
        }
...
static int air724_close_apn(void)
{
    ...
    at_echo_create(&echo, echo_buffer, sizeof(echo_buffer), NULL);
    at_cmd_exec(&echo, 3000, "AT+CIPSHUT\r\n");
...
{
#if CFG_MODULE_SINGLE_LINK_EN > 0u	
    int try = 0;
    at_echo_t echo;

    while (try++ < 10) {
        at_echo_create(&echo, NULL, 0, NULL);
        at_cmd_exec(&echo, 300, "AT+CIPMODE=%d\r\n", mode == SAL_SEND_MODE_NORMAL ? 0 : 1);
        if (echo.status == AT_ECHO_STATUS_OK) {
            return 0;
        }
    }
...
static int air724_set_apn(void)
{
    ...
    at_echo_create(&echo, NULL, 0, NULL);
    at_cmd_exec(&echo, 300, "AT+CSTT=\"CMNET\"\r\n");
    if (echo.status != AT_ECHO_STATUS_OK) {
        return -1;
    }

	at_echo_create(&echo, NULL, 0, NULL);	
    at_cmd_exec(&echo, 50000, "AT+CIICR\r\n");
    if (echo.status != AT_ECHO_STATUS_OK) {
        return -1;
    }	
	
    at_echo_create(&echo, NULL, 0, NULL);	
    at_cmd_exec(&echo, 300, "AT+CIFSR\r\n");
...
static int air724_connect(const char *ip, const char *port, sal_proto_t proto)
{
    ...
    id = at_channel_alloc(ip, port);
    if (id == -1) {
        return -1;
    }
		
    at_echo_create(&echo, NULL, 0, NULL);	
    at_cmd_exec(&echo, 2000, "AT+CIPHEAD=1\r\n");
    if (echo.status != AT_ECHO_STATUS_OK) {
        at_channel_free(id);
        return -1;
    }
...
#if CFG_MODULE_SINGLE_LINK_EN > 0u
    at_echo_create(&echo, NULL, 0, "CONNECT OK");
    at_cmd_exec_until(&echo, 4000, "AT+CIPSTART=%s,%s,%s\r\n",
                        proto == SAL_PROTO_UDP ? "UDP" : "TCP", ip, atoi(port));
    ...
    at_echo_create(&echo, NULL, 0, result);
    at_cmd_exec_until(&echo, 8000, "AT+CIPSTART=%d,%s,%s,%d\r\n",
                        id, proto == SAL_PROTO_UDP ? "UDP" : "TCP", ip, atoi(port));
    if (echo.status != AT_ECHO_STATUS_EXPECT) {
        at_channel_free(id);
        return -1;
    }
...
{
    return at_channel_read_timed(id, buf, len, timeout);
}
...
int air724_send(int id, const void *buf, size_t len)
{
    ...
    at_echo_create(&echo, NULL, 0, expect_str);
    at_raw_data_send_until(&echo, 10000, (uint8_t *)buf, len);
    if (echo.status != AT_ECHO_STATUS_EXPECT) {
        at_global_lock_post();
        return -1;
    }
...
int air724_sendto(int id, char *ip, char *port, const void *buf, size_t len)
{
    ...
    at_raw_data_send_until(&echo, 1000, (uint8_t *)buf, len);
    if (echo.status != AT_ECHO_STATUS_EXPECT) {
        at_global_lock_post();
        return -1;
    }
...
#if CFG_MODULE_SINGLE_LINK_EN > 0u
    at_cmd_exec(&echo, 1000, "AT+CIPCLOSE\r\n");
#else
    at_cmd_exec(&echo, 1000, "AT+CIPCLOSE=%d\r\n", id);
#endif
...
static int air724_parse_domain(const char *host_name, char *host_ip, size_t host_ip_len)
{
    ...
    at_echo_create(&echo, NULL, 0, NULL);
    at_cmd_exec(&echo, 1000, "AT+CDNSGIP=\"%s\"\r\n", host_name);

    if (echo.status != AT_ECHO_STATUS_OK) {
        return -1;
    }
    
    sem_pend(&domain_parser_sem, TIME_FOREVER);
...
#define MIN(a, b)   ((a) < (b) ? (a) : (b))
        ...
        if (at_uart_read(buffer, read_len) != read_len) {
            return;
        }

        if (at_channel_write(channel_id, buffer, read_len) <= 0) {
            return;
        }
...
int air724_sal_init(hal_uart_port_t uart_port)
{
    ...
    if (at_init(uart_port, air724_at_event,
                        sizeof(air724_at_event) / sizeof(air724_at_event[0])) != 0) {
        return -1;
    }

    if (sal_module_register(&sal_module_air724) != 0) {
        return -1;
    }
    if (sal_module_init() != 0) {
        return -1;
    }
