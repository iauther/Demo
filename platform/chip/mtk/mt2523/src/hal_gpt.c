...
//原有设置定时器只能设置整数毫秒数，不能设置256Hz这样的采样率，增加一个能设置采样率的函数
hal_gpt_status_t hal_gpt_start_timer_Hz(hal_gpt_port_t gpt_port, uint32_t timeout_time_Hz, hal_gpt_timer_type_t timer_type)
{
    ...
    /* set to 32K clock and 1 division */
    gp_gpt[gpt_port]->GPT_CLK       = GPT_CLOCK_32KHZ | (uint32_t)GPT_DIVIDE_1;
    gp_gpt[gpt_port]->GPT_COMPARE   = 32768 / timeout_time_Hz-1;
    gp_gpt[gpt_port]->GPT_IRQ_ACK   = GPT_IRQ_FLAG_ACK;
