...
#ifdef MG_ENABLE_LINES
#line 1 "src/drivers/ecxx.c"
#endif

#if MG_ENABLE_TCPIP && defined(MG_ENABLE_DRIVER_ECXX) && MG_ENABLE_DRIVER_ECXX


static size_t ecxx_rx(void *buf, size_t buflen, struct mg_tcpip_if *ifp) {
  struct mg_tcpip_urt *urt = (struct mg_tcpip_urt *) ifp->driver_data;
  return urt->tcp_rx(buf, buflen);
}

static size_t ecxx_tx(const void *buf, size_t buflen, struct mg_tcpip_if *ifp) {
  struct mg_tcpip_urt *urt = (struct mg_tcpip_urt *) ifp->driver_data;
  return urt->tcp_tx((void*)buf, buflen);
}

static bool ecxx_init(struct mg_tcpip_if *ifp) {
  struct mg_tcpip_urt *urt = (struct mg_tcpip_urt *) ifp->driver_data;
  return 1;
}

static bool ecxx_up(struct mg_tcpip_if *ifp) {
  struct mg_tcpip_urt *urt = (struct mg_tcpip_urt *) ifp->driver_data;
  return 1;
}

struct mg_tcpip_driver mg_tcpip_driver_ecxx = {ecxx_init, ecxx_tx, ecxx_rx,
                                                ecxx_up};
#endif
