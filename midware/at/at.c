...
int at_global_lock_pend(at_agent_t *at_agent)
{
...
int at_global_lock_post(at_agent_t *at_agent)
{
...
static int at_uart_getchar_from_fifo(at_agent_t *at_agent, uint8_t *data)
{
    TOS_CPU_CPSR_ALLOC();
    k_err_t err;
...
static int at_uart_getchar(at_agent_t *at_agent, uint8_t *data, k_tick_t timeout)
{
...
static at_event_t *at_event_do_get(at_agent_t *at_agent, char *buffer, size_t buffer_len)
{
    int i = 0;
...
static at_event_t *at_get_event(at_agent_t *at_agent)
{
    char *buffer;
    size_t buffer_len;
...
int at_uart_read(at_agent_t *at_agent, uint8_t *buffer, size_t buffer_len)
{
    uint8_t data;
    size_t read_len = 0;
...
int at_uart_readline(at_agent_t *at_agent, uint8_t *buffer, size_t buffer_len)
{
    uint8_t data;
    size_t read_len = 0;
...
int at_uart_drain(at_agent_t *at_agent, uint8_t *buffer, size_t buffer_len)
{
    uint8_t data;
    size_t read_len = 0;
...
static int at_is_echo_expect(at_agent_t *at_agent)
{
...
static at_parse_status_t at_uart_line_parse(at_agent_t *at_agent)
{
    size_t curr_len = 0;
...
static int at_uart_send(at_agent_t *at_agent, const uint8_t *buf, size_t size, uint32_t timeout)
{
    int ret;
...
int at_echo_create(at_echo_t *echo, char *buffer, size_t buffer_size, char *echo_expect)
{
    if (!echo) {
        return -1;
    }
...
int at_echo_fuzzy_matching_create(at_echo_t *echo, char *buffer, size_t buffer_size, char *echo_expect_contains)
{
    if (!echo) {
        return -1;
    }
...
int at_raw_data_send(at_agent_t *at_agent, at_echo_t *echo, uint32_t timeout, const uint8_t *buf, size_t size)
{
    int ret = 0;
    ...
    if (at_global_lock_pend(at_agent) != 0) {
        return -1;
    }
    ...
    at_global_lock_post(at_agent);

    return ret;
}

int at_raw_data_send_until(at_agent_t *at_agent, at_echo_t *echo, uint32_t timeout, const uint8_t *buf, size_t size)
{
    int ret = 0;
    ...
    at_port_sem_free(&echo->__expect_notify);
...
static int at_cmd_do_exec(at_agent_t *at_agent, const char *format, va_list args)
{
    size_t cmd_len = 0;
...
int at_cmd_exec(at_agent_t *at_agent, at_echo_t *echo, uint32_t timeout, const char *cmd, ...)
{
    int ret = 0;
    va_list args;
...
int at_cmd_exec_until(at_agent_t *at_agent, at_echo_t *echo, uint32_t timeout, const char *cmd, ...)
{
    int ret = 0;
    va_list args;
...
static int at_recv_cache_init(at_agent_t *at_agent)
{
...
static void at_recv_cache_deinit(at_agent_t *at_agent)
{
    uint8_t *buffer = K_NULL;

    buffer = at_agent->recv_cache.buffer;
    if (buffer) {
        AT_PORT->mem_free(buffer);
    }
...
static at_data_channel_t *at_channel_get(at_agent_t *at_agent, int channel_id, int is_alloc)
{
    /*
...
int at_channel_read(at_agent_t *at_agent, int channel_id, uint8_t *buffer, size_t buffer_len)
{
    int read_len;
...
int at_channel_read_timed(at_agent_t *at_agent, int channel_id, uint8_t *buffer, size_t buffer_len, uint32_t timeout)
{
    int read_len = 0;
    size_t total_read_len = 0;
...
int at_channel_write(at_agent_t *at_agent, int channel_id, uint8_t *buffer, size_t buffer_len)
{
    int ret;
    at_data_channel_t *data_channel = K_NULL;
...
errout:
    AT_PORT->mem_free(fifo_buffer);
    return -1;
}

int at_channel_alloc_id(at_agent_t *at_agent, int channel_id, const char *ip, const char *port)
{
    at_data_channel_t *data_channel = K_NULL;
...
int at_channel_alloc(at_agent_t *at_agent, const char *ip, const char *port)
{
    int id = 0;
    at_data_channel_t *data_channel = K_NULL;
    size_t socket_buffer_size = 0;
...
int at_channel_alloc_with_size(at_agent_t *at_agent, const char *ip, const char *port, size_t socket_buffer_size)
{
    int id = 0;
    at_data_channel_t *data_channel = K_NULL;
...
int at_channel_free(at_agent_t *at_agent, int channel_id)
{
    at_data_channel_t *data_channel = K_NULL;
    ...
    AT_PORT->mtx_free(&data_channel->rx_lock);

    tos_stopwatch_destroy(&data_channel->timer);

    AT_PORT->mem_free(data_channel->rx_fifo_buffer);
...
int at_channel_set_broken(at_agent_t *at_agent, int channel_id)
{
    at_data_channel_t *data_channel = K_NULL;
...
int at_channel_is_working(at_agent_t *at_agent, int channel_id)
{
    at_data_channel_t *data_channel = K_NULL;
...
static void at_channel_init(at_agent_t *at_agent)
{
    int i = 0;
...
static void at_channel_deinit(at_agent_t *at_agent)
{
    int i = 0;

    for (i = 0; i < AT_DATA_CHANNEL_NUM; ++i) {
        at_channel_free(at_agent, i);
    }
}

const char *at_channel_ip_get(at_agent_t *at_agent, int channel_id)
{
    at_data_channel_t *data_channel = K_NULL;
...
const char *at_channel_port_get(at_agent_t *at_agent, int channel_id)
{
    at_data_channel_t *data_channel = K_NULL;
...
static void at_event_table_set(at_agent_t *at_agent, at_event_t *event_table, size_t event_table_size)
{
...
static void at_uart_input_notify(at_agent_t *at_agent)
{
    tos_sem_post(&at_agent->uart_rx_sem);
}

static void idle_check_timer_callback(void *args)
{
    at_agent_t *at_agent = (at_agent_t *)args;
    at_uart_input_notify(at_agent);
}
#endif  /* #if AT_INPUT_SIMULATE_IDLE_EN */

int at_init(at_agent_t *at_agent, char *task_name, k_stack_t *stk, hal_uart_port_t uart_port, at_event_t *event_table, size_t event_table_size)
{
    void *buffer = K_NULL;
...
errout8:
    ...
    AT_PORT->mtx_free(&at_agent->global_lock);

errout7:
    AT_PORT->mtx_free(&at_agent->uart_tx_lock);

errout6:
//    AT_PORT->mtx_free(&at_agent->uart_rx_lock);
...
#if AT_INPUT_TYPE_FRAME_EN
errout4:
    AT_PORT->mem_free(at_agent->uart_rx_frame_mail_buffer);
...
errout2:
    // AT_PORT->mtx_free(&at_agent->cmd_buf_lock);

//errout1:
    AT_PORT->mem_free(at_agent->cmd_buf);
    at_agent->cmd_buf = K_NULL;

errout0:
    AT_PORT->mem_free(at_agent->uart_rx_fifo_buffer);
...
void at_deinit(at_agent_t *at_agent)
{
    tos_task_destroy(&at_agent->parser);

    AT_PORT->io_free(&at_agent->uart);
    ...
    at_agent->uart_rx_frame_mail_buffer = K_NULL;
#else
    AT_PORT->sem_free(&at_agent->uart_rx_sem);
#endif /* AT_INPUT_TYPE_FRAME_EN */
...
#if AT_INPUT_TYPE_FRAME_EN
void at_uart_input_frame(at_agent_t *at_agent, uint8_t *pdata, uint16_t len)
{
    int ret;
...
#elif AT_INPUT_SIMULATE_IDLE_EN
void at_uart_input_byte_no_notify(at_agent_t *at_agent, uint8_t data)
{
...
#else
void at_uart_input_byte(at_agent_t *at_agent, uint8_t data)
{
