...
#define SD_CLK_DIV_TRANS                    ((uint16_t)0x0008)        //change it from 2 to 8
...
int dal_sd_init(void)
{
    ...
    st = sd_init();
    if(SD_OK == st){
        st = sd_card_information_get(&sd_cardInfo);
    }
    ...
    if ((SD_OK == st) && (!locked)) {
        st = sd_bus_mode_config(SDIO_BUSMODE_4BIT);
    }
    if (SD_OK == st) {
        st = sd_transfer_mode_config(SD_DMA_MODE);
        //st = sd_transfer_mode_config(SD_POLLING_MODE);
    }
...
int dal_sd_read(U8 *buf, U32 sector, U32 count)
{
    sd_error_enum st;
    
    if(count==1) {
        st = sd_block_read((U32*)buf, sector*sd_cardInfo.card_blocksize, sd_cardInfo.card_blocksize);
    }
    else {
        st = sd_multiblocks_read((U32*)buf, (sector*sd_cardInfo.card_blocksize), sd_cardInfo.card_blocksize, count);
    }
...
int dal_sd_write(U8 *buf, U32 sector, U32 count)
{
    sd_error_enum st;
    
    if(count==1) {
        st = sd_block_write((U32*)buf, sector*sd_cardInfo.card_blocksize, sd_cardInfo.card_blocksize);
    }
    else {
        st = sd_multiblocks_write((U32*)buf, (sector*sd_cardInfo.card_blocksize), sd_cardInfo.card_blocksize, count);
    }
...
int dal_sd_status(void)
{
    U32 st;
    sd_error_enum err;
    
    err = sd_cardstatus_get(&st);
    if(err!=SD_OK) {
        return -1;
    }
...
int dal_sd_test(void)
{
    ...
    wbuf = (U32*)malloc(cnt*4);
    rbuf = (U32*)malloc(cnt*4);
    for(i = 0; i < cnt; i++) {
        wbuf[i] = i;
    }
    
#if 0
    err = sd_block_write(wbuf, 1*512, 512);
    if(SD_OK == err) {
        err = sd_block_read(rbuf, 1*512, 512);
    }
#else
    err = sd_multiblocks_write(wbuf, 1*512, 512, 2);
    if(SD_OK == err) {
        err = sd_multiblocks_read(rbuf, 1*512, 512, 2);
    }
#endif
