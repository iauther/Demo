...
//返回值:0,成功
//    其他,错误代码
int dal_nand_read_page(u32 PageNum,u16 ColNum,u8 *pBuffer,u16 NumByteToRead)
{
    vu16 i=0;
	u8 res=0;
...
//返回值:0,成功
//    其他,错误代码
int dal_nand_read_page_cmp(u32 PageNum,u16 ColNum,u32 CmpVal,u16 NumByteToRead,u16 *NumByteEqual)
{
    u16 i=0;
	u8 res=0;
    *(vu8*)(NAND_ADDRESS|NAND_CMD)=NAND_AREA_A;
...
//返回值:0,成功
//    其他,错误代码
int dal_nand_write_page(u32 PageNum,u16 ColNum,u8 *pBuffer,u16 NumByteToWrite)
{
    vu16 i=0;
	u8 res=0;
	u8 eccnum=0;		//需要计算的ECC个数，每NAND_ECC_SECTOR_SIZE字节计算一个ecc
...
//返回值:0,成功
//    其他,错误代码
int dal_nand_read_spare(u32 PageNum,u16 ColNum,u8 *pBuffer,u16 NumByteToRead)
{
    int r;
    u8 remainbyte=0;
    ...
    r = nand_read_page(PageNum,ColNum+nand_attr.page_mainsize,pBuffer,NumByteToRead);//读取数据
    return r;
}
...
//返回值:0,成功
//    其他,失败
int dal_nand_write_spare(u32 PageNum,u16 ColNum,u8 *pBuffer,u16 NumByteToWrite)
{
    int r;
    u8 remainbyte=0;
    remainbyte=nand_attr.page_sparesize-ColNum;
    ...
    r = nand_write_page(PageNum,ColNum+nand_attr.page_mainsize,pBuffer,NumByteToWrite);//读取
    return r;
}
...
//返回值:0,擦除成功
//    其他,擦除失败
int dal_nand_erase_block(u32 BlockNum)
{
...
nand_attr_t* dal_nand_get_attr(void)
{
    return &nand_attr;
}
